...
function SimpleMovement:new(base_x,base_y,base_v)
	...
	self.base_dai = 230*(game.scale/forZoomingIn)	--Idle area.
	self.base_cfd = 0		--Use for mouse cursor to game.middleX,game.middleY distance.(forPlayer)
	self.base_damv = 710*(game.scale/forZoomingIn)	--Max distance allowed to limit character's velocity.
...
function SimpleMovement:update(dt,animal_x,animal_y,food_x,food_y)
	...
	if self.base_cfd > self.base_dai and self.base_cfd < self.base_damv then
		self.base_x = self.base_x + self.base_v*self.base_cos*(self.base_cfd*forZoomingIn/50)*dt
		self.base_y = self.base_y + self.base_v*self.base_sin*(self.base_cfd*forZoomingIn/50)*dt
	elseif self.base_cfd >= self.base_damv then
		self.base_x = self.base_x + self.base_v*self.base_cos*(self.base_da*forZoomingIn/50)*dt
		self.base_y = self.base_y + self.base_v*self.base_sin*(self.base_da*forZoomingIn/50)*dt
	end
end
