...
function SimpleMovement:new(base_x,base_y,base_v)
	...
	self.base_v = base_v or 35  -- game.scale doesnt have to be multiplied by 'forZoomIn'
					...
					--  by multiplying it with forZoomIn variable and by
					--  doing that removes game.scale*forZoomIn/forZoomIn,
					--  forZoomIn gets cancelled thus maintaining its ratio
					--  even after getting zoomIn
	self.base_dai = (630)*(game.scale)	-- Idle area.
	self.base_cfd = 0		-- Use for mouse cursor to game.middleX,game.middleY
					-- distance.(forPlayer)
	self.base_damv = (710)*(game.scale)	-- Max distance allowed to limit character's
					--  velocity.
