IsometricInteract = Object:extend() -- Colliderless, interactable.
	--November 9 2022 : Only effective for smaller spawns of isometricClick objects,
	--for now.
	--..pretty much useless for objects like isometricClick objects with a ratio of
	--a footlong.
		--EXPLAINED IN 
...
function IsometricClick:new(x,y,ll,rl,scale,ox,oy)
	self.ox = ox or 0		--This offsets are only used on
					-- IsometricClick.update() 	November 10 2022
	self.oy = oy or 0		-- Have not been used yet, but its purpose was to 
					-- move the interactive area like from CursorHover()
...
function IsometricClick:update(dt) -- Exist to stay consistent with
				   -- Environment.update(dt) loops
end
...
--	love.graphics.setColor(255,255,255)
			...
			--- CHANGE OF PLAN NOVEMBER 10 2022,
	--maybe under the function below is where I should add the offset value for
	--interactable area
	-- of the isometric(either that works with colliders or not/isometricBeta.lua)
	-- I will be doing it this way "implent-ing kinda way" since I want to re-use
	-- sorting-order function in environment.lua for isometric
	-- that has collider with objects that is isometric that doesn't want anything to do
	-- with colliders functions on humanColldersFunctions.lua
...
--	Player.evilCursorY = self.m*(Player.evilCursorX-self.x) + self.y
	...
	if Player.evilCursorX ~= Player.SelectedCharacter.base_x and Player.evilCursorY ~= Player.SelectedCharacter.base_y then
		--If-statement above added so that this function won't run unless
		--the cursor is actively being used.(Does not mean "Press" though)
		--	Unlikey scenario that it would cause problem in the game though,
		--	so this pass my vibe check.
