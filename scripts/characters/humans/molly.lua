...
function Molly:new()
	...
	--November 4 2022 : Attempt at collider, Has to manually add values and just multiplied it with self.scale
	--Just a reminder that those circles should be an ellipse in isometric view, but for alpha test, circle it is :)
	--It may be redundant though since its particularly small radius and would be not so noticable, maybe...
	--	This is not a professional fighting game like Tekken VIshnu sake! ..bruh, imagine the hitbox of that thing.
	self.headOffsetX,self.headOffsetY = 0*self.scale,0*self.scale--notYet		--Offset of those hitboxes
	self.bodyOffsetX,self.bodyOffsetY = 0*self.scale,0*self.scale--notYet		--Variables directly called on humanColliderFunctions.lua
	...
	self.bodyw,self.bodyh = 60*self.scale,60*self.scale				--		body, width&height, rectangle.
...
function Molly:update(dt)
	...
	end		---November 3 2022, Loop through Environment.floors()to find specific sfx to play ..it doesn't exists yet.
end
